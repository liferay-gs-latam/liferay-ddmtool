...
var saveTemplates = function(e, filesRootPath, options) {
	...
	for (var i = 0; i < e.length; ++i) {
		...
		// structure.
		if (entry.classPK > 0) {
			curStructureClassNameId = Structures.getSingleValue('structureId', entry.classPK, 'classNameId');
			...
			friendlyName = LrClassNameConfig.getSingleValue('id', entry.classNameId, 'friendlyName');
		}

		filePath = filePath + '/templates/' + entry.nameCurrentValue + '.' + entry.language;
		fileContent = entry.script;
