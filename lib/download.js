...
var download = {
	...
	getClassNameIds: function () {
		...
		for (var i = 0; i < LrClassNameConfig.fetch().length; i++) {
			utilities.writeToScreen('classNameId: ' + LrClassNameConfig.fetch(i, 'clazz'), Constants.fetch('SEVERITY_NORMAL'), Constants.fetch('SCREEN_PRINT_INFO'));
	...
	getStructures: function () {
		...
		for (i = 0; i < LrClassNameConfig.fetch().length; ++i) {
			utilities.writeToScreen('Fetching ' + LrClassNameConfig.fetch(i, 'id').classNameId, Constants.fetch('SEVERITY_NORMAL'), Constants.fetch('SCREEN_PRINT_INFO'));
			payload.push('{"/ddm.ddmstructure/get-structures": {"companyId": ' + CompanyId + ', "groupIds": [' + sitesList.join() + '], "classNameId": ' + LrClassNameConfig.fetch(i, 'id').classNameId + ', "status": 0}}');
		}

		return getData('[' + payload.join() + ']').then(
			function (e) {

				// utilities.writeToScreen('All ' + LrClassNameConfig.getAllFilter(), Constants.fetch('SEVERITY_NORMAL'), Constants.fetch('SCREEN_PRINT_INFO'));

				// Make sure we have all the structures in our LrClassNameConfig
				// If not, warn the user about it!
				for (var i = 0; i < e.length; ++i) {

					utilities.writeToScreen('Checking ' + e[i].id.classNameId + ", class: " + e[i].id.value, Constants.fetch('SEVERITY_NORMAL'), Constants.fetch('SCREEN_PRINT_INFO'));
		...
		for (var i = 0; i < Sites.fetch().length; ++i) {
			for (var ii = 0; ii < LrClassNameConfig.fetch().length; ii++) {
				if (LrClassNameConfig.fetch(ii, 'getTemplate')) {
					payload.push('{"/ddm.ddmtemplate/get-templates": {"groupId": ' + Sites.fetch(i, 'groupId') + ', "classNameId": ' + LrClassNameConfig.fetch(ii, 'id') + '}}');
				}
			}
		}
