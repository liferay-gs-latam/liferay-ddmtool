...
var download = {
	...
	getStructures: function () {
		...
		for (i = 0; i < LrClassNameConfig.fetch().length; ++i) {
			...
			payload.push('{"/ddm.ddmstructure/get-structures": {"companyId": ' + CompanyId + ', "groupIds": [' + sitesList.join() + '], "classNameId": ' + LrClassNameConfig.fetch(i, 'id') + ', "status": 0}}');
		}
		...
		for (var i = 0; i < Sites.fetch().length; ++i) {
			for (var ii = 0; ii < LrClassNameConfig.fetch().length; ii++) {
				if (LrClassNameConfig.fetch(ii, 'getTemplate')) {
					...
					for (var x = 0; x < LrClassNameConfig.fetch().length; x++) {
						if (LrClassNameConfig.fetch(x, 'isNativeDDM')) {
						...
						console.log('Checking {"/ddm.ddmtemplate/get-templates": {"companyId": ' + CompanyId + 
							...
							', "resourceClassNameId": ' + LrClassNameConfig.fetch(x, 'id') + 
							', "status": 0}}');
