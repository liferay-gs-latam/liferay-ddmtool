...
function getSite (validStructures, fileFriendlyName, className, isADT, returnObj) {
	...
	if (possibleSites.length === 0) {
		...
		possibleSites.forEach(function(entry) {
			questionsSites.push({
				name: Sites.getSingleValue('groupId', entry, 'descriptiveName') + ' [' + LrClassNameConfig.getSingleValue('id', Sites.getSingleValue('groupId', entry, 'classNameId'), 'friendlyName')+ ']',
				value: entry
			});
		});
	...
	getSite(newObj.validStructures, newObj.fileFriendlyName, newObj.className, newObj.isADT, returnObj)
		.then(function (currentSite) {
			...
			.then(function (currentSiteAndStructure) {
				...
				returnObj.group.name			= Sites.getSingleValue('groupId', currentSiteAndStructure.site, 'descriptiveName');
				...
				var resourceClassNameId = LrClassNameConfig.getSingleValue("clazz", 
					(newObj.isADT) ? "com.liferay.portlet.display.template.PortletDisplayTemplate" : "com.liferay.journal.model.JournalArticle", "id");
				...
				returnObj.payload = '{"/ddm.ddmtemplate/add-template": ' + JSON.stringify(payload) + '}';
